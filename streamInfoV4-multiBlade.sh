...
do
	...
	channelName=`echo $INPUT | awk -F "|" '{print $4}'`	
	...
	do	
		...
		LOG_FILE=$LOG/$channelName-$BladeIP.log	
		/usr/bin/sshpass -p 'password' /usr/bin/ssh  -o ConnectTimeout=10  -tt -o 'StrictHostKeyChecking no' root@$BladeIP -p 22 "ffmpeg -timeout 30000 -i \"udp://$MulticastIP:$UdpPort?fifo_size=50000000&overrun_nonfatal=1&buffer_size=50000000&localaddr=$ETH1\"  2>&1 |  sed -n '/Program '$ProgramNo'\b/,/Program/{/Program/!p}'" > $LOG_FILE 2>/dev/null
